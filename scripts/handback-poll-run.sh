...
Everything else in the skill still applies in full and must not be weakened: the gate sits BEFORE the action; step 2's tier-1 labelling pass on every item you work, submitting tier-1 redos with --tier1; the carry_out intent/verify-first/complete discipline, including the never-automated list (payments, credentials, signatures, phone calls) which are parked and listed in parkedFlags, never retried (when a carry-out needs a sign-in only Kevin can do, annotate the task with a note that OPENS with 'PARKED:' naming the site: the queue then rests it for a day instead of waking you every half hour); the MINOR-EDITS branch on every carry_out (an Approved with minor edits carrying a note means Kevin typed an edit: apply only that change, run the revise subcommand, and carry out the REVISED text — complete refuses otherwise); the mandatory closing 'Carrying this out will involve:' line under 400 characters on every redo; and step 4b's CEO review pass over non-tier-1 redos before they are submitted.
