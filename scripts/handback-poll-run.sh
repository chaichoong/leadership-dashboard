...
# --- free half: the blocker loop (Kevin, 25 Sep 2026) ----------------------
# A task an agent could not finish carries a BLOCKER OPEN line with its kind
# (SIGN-IN, SITE, TOOL, KEVIN). This clears every wall whose cause is gone (the
# site is on the robot's list now, the TOOL finding closed fixed) and hands the
# task back WITH Kevin's verdict, so the queue read below already sees it.
# Before this, a fix never woke anything: 6 Chedburgh Place stayed parked after
# TopCashback was added and Kevin had signed in. No model tokens. The output is
# the Estate status row's source; a failure is loud but never stops hand-backs.
if ! /usr/bin/python3 "$REPO/scripts/agent-dispatch.py" blockers --sweep > "$SCRATCH/blockers.json" 2>"$SCRATCH/blockers.err"; then
  echo "WARNING: blocker sweep failed — a fixed wall may not have woken its task" >&2
  tail -c 500 "$SCRATCH/blockers.err" >&2
fi
...
WORK ONLY THE HAND-BACKS. From the worklist, work every item whose kind is carry_out (Approval Outcome is an Approved kind) or redo (Approval Outcome is Changes requested), and every item carrying signinReopened (Kevin signed the robot in for it: its newest note says SIGNED IN; their ids are also listed top-level as signinReopened). For a signinReopened item run node scripts/agent-browser.js session --site <host> FIRST. An item whose Approval Outcome is an Approved kind (a carry-out a sign-in woke: its last note says BLOCKER CLEARED (SIGN-IN ...)) is CARRIED OUT and closed with complete (or complete --keep-open when the approval says it stays open), NEVER submitted: a submit wipes Kevin's approval and asks him again. If its site is still signed out, record it with python3 scripts/agent-dispatch.py block TASKID --kind SIGN-IN --subject <host> --why \"<what you saw>\" and stop. Any other signinReopened item: if signedIn is true carry on from where the agent stopped and submit the finished work; if it is false, say so with the single line SIGN-IN NEEDED: <site> (<login url>) and stop. IGNORE new work entirely, do NO routing and NO escalation — those belong to the 09:00/13:00/17:00 slots and to daily-ops, and re-deciding them every thirty minutes would burn Kevin's tokens on questions nobody has answered since the last tick. If the only items left are new work, write the report and stop.

Everything else in the skill still applies in full and must not be weakened: the gate sits BEFORE the action; step 2's tier-1 labelling pass on every item you work, submitting tier-1 redos with --tier1; the carry_out intent/verify-first/complete discipline, including the never-automated list (payments, credentials, signatures, phone calls) which are parked and listed in parkedFlags, never retried (when a carry-out hits a wall, record it with the block subcommand and its kind, never a PARKED note, which annotate now refuses: SIGN-IN for a signed-out site on the robot's list, SITE for a site the list cannot reach, TOOL for the robot's own setup being broken, KEVIN for a payment, purchase, signature, credential, identity or physical step; the queue rests the task for a day, the fix is routed to whoever clears it, and the task wakes by itself when it is fixed. A task whose last Note says BLOCKER CLEARED is to be FINISHED from where it stopped. complete refuses a blocked task: clear a KEVIN wall only with unblock --evidence once you have SEEN the proof); the MINOR-EDITS branch on every carry_out (an Approved with minor edits carrying a note means Kevin typed an edit: apply only that change, run the revise subcommand, and carry out the REVISED text — complete refuses otherwise); the mandatory closing 'Carrying this out will involve:' line under 400 characters on every redo; and step 4b's CEO review pass over non-tier-1 redos before they are submitted.
