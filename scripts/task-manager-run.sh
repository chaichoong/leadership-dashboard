...
# PUBLIC repo, so anything content-bearing that lands there is quarantined
# and the run fails loudly (same rule as inbound-triage).
set -u
...
"$CLAUDE" -p "You are the Task Manager agent's scheduled run (one of the 09:00 / 13:00 / 17:00 slots). Do this skill in full: /Users/kevinbrittain/.claude/scheduled-tasks/task-manager-board/SKILL.md
...
# monitoring/. Task payload markers: description JSON or the inbound content
# field name. Quarantining alone is not enough — the run FAILS so the
# leak-shaped behaviour gets fixed, not absorbed.
__LEAKED=""
while IFS= read -r f; do
  [ -z "$f" ] && continue
  mv "$f" "$SCRATCH/" && __LEAKED="$__LEAKED $f"
done < <(grep -rlE '"description":|Inbound Message Content|CREDITOR MATTER' "$REPO/monitoring/" 2>/dev/null || true)

__TAIL=$(tail -n +$((__START_LINE + 1)) "$LOG" 2>/dev/null)
__BAD=$(printf '%s\n' "$__TAIL" | grep -E '401|Unauthorized|OAuth access token has expired|BROKEN|VERIFY FAIL' || true)
