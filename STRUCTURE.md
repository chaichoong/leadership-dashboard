...
| `scripts/airtable-automations/` | Source of truth for the Airtable AUTOMATION scripts (`bms-record-generator.js`, `transaction-to-bms-link.js`). Airtable's REST API refuses to edit `customScript` nodes ("read-only node ... edit in the Airtable UI instead"), so these are the reviewable copy and the live version is pasted in by hand and published with the Update button. Keep both in step. **Write them in pure ASCII**: Airtable's editor mangles pasted UTF-8 (an em dash became `—`), and the BMS key separator is an EN DASH, so it is written as `"\u2013"` rather than literally. | New Airtable automation script |
