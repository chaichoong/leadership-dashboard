...
| `scripts/` | Build/automation scripts (`pre-commit`, Slack automation, `agent-accuracy-report.py` — what the CEO huddle reads out about agent accuracy). `scripts/slack-automation/` holds the Cloudflare Workers: `contractor-bot.js` (Slack app + the AI-agent approval loop cron) and its `approvals.js` module, `money-daily-worker.js` (09:00 CEO brief), `notify-slack-worker.js` | New automation script. Apps Script sources (`*.gs`) also belong here |
