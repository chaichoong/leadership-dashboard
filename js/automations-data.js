...
var AUTOMATIONS = {
    ...
    macJobs: [
        ...
        { key: 'estate-status', name: 'Estate Status Board', when: 'every 10 minutes', status: 'on',
          what: 'Writes one line per scheduled job to the Estate Status table: when it last ran, whether it worked, why not in plain words, and when it is next due, plus the list of tasks that are not moving. The Estate status tab on the AI Agents page reads it. If this job stops, that tab says so.' },
        { key: 'tenant-leads', name: 'Tenant-finding chain', when: '8:10am daily', status: 'on',
          what: 'Finds rooms that need a tenant (a void, a notice, or a room move adopted on the Growth Plan) and keeps single people aged 35 or over on Universal Credit coming in. It drafts one email card for your queue to the council, charity and support contacts near the room, one to our tenants about the £50 referral bonus, and a monthly check-in to people on the list. It gives Roy the adverts to post and a list of qualified people to call for viewings. It screens every sign-up from the form. When Roy says someone wants a room, it gives him a move-in checklist (right to rent, proof of Universal Credit, a reference, the signing, then the works and the move-in date) and drafts one email card asking that person for their documents. It checks each house with an open room against the compliance book, so nobody new moves in before the house is legal, and puts a £50 bonus on the Friday Payment Run when a referred tenant pays their first rent. It writes the Tenant-finding chain line on the Estate status board and one line in your 09:00 brief, which turn red when a step that should have happened did not.' },
        { key: 'estate-drift', name: 'CEO and Board Currency Check', when: '6:25am daily', status: 'on',
          what: 'Checks that the AI CEO, the eleven department heads, the five workers, the CEO skills, the morning huddle instructions, the brain files the CEO reads first and the 9am brief are all describing how the business runs TODAY. It looks for wording a decision has retired (for example routing work to Mica after that stopped, or the old spending thresholds) and for any decision newer than the CEO\'s own "as at" page. Red means a prompt is out of date and gets fixed the same day. Built the day Kevin said he had stopped using his CEO because he felt outdated.' },
        { key: 'content-engine', name: 'Content Engine folder watch', when: '2:00am daily', status: 'on',
          what: 'Looks in the Runpreneur raw video folder on Drive for clips it has not seen, creates one "Episode N Full Episode" record per new shooting day (New Upload) and copies the oldest waiting clip to this Mac, then turns one pulled clip into the three Runpreneur videos (full 16:9 with captions, LFMD and Summary 9:16 with the banner), files them in the edited Drive folder, writes the links and transcript onto the episode record, then writes the platform copy for that episode in Kevin\'s voice using the Content Machine\'s own prompts and checks it against the writing rules, and when an episode has its video, thumbnail and copy it puts ONE approval card in Kevin\'s queue (counted by the 8am digest) and copies his verdict back onto the episode. For episodes Kevin has APPROVED it schedules the full episode to YouTube and, the night after YouTube publishes, the Summary and Learnings clips to Facebook, LinkedIn, TikTok (and Instagram, Threads once connected) through GoHighLevel, publishes the blog article on runpreneur.org.uk, then writes the published links back, and redraws the How far I\'ve run map page on the website from Strava (route marker, countries, facts). The full episode carries the branded intro after Kevin\'s sign-off, the podcast upload plan is written for the browser lane, and the website\'s "How far I\'ve run" numbers and the Strava run name are updated from Strava and Stripe. Nothing is scheduled without his approval on the card.' },
        { key: 'content-engine-publish', name: 'Content Engine daytime publisher', when: 'hourly 7:15am to 8:15pm', status: 'on',
          what: 'Every hour in the day it reads Kevin\'s verdicts on the episode cards, puts the next approved episode (in day order) on YouTube, and once YouTube gives the link it schedules that episode\'s clips with their copy to the socials, the blog article and the podcast for later the same day. No rendering; the night job does that.' },
        ...
        { key: 'knowledge-os-sort', name: 'Knowledge Sorter', when: '8:50am daily', status: 'on',
        ...
        { key: 'job-digest', name: 'Job Digest', when: '11:00am daily', status: 'on',
          what: 'One summary of every scheduled job and whether it ran, was skipped, or failed. This is how a job that quietly stopped gets noticed. Since 1 Sep 2026 it writes to its log only — no Slack (Kevin retired system alerts).' },
        { key: 'daily-ops-guard', name: 'Daily Sweep Guard', when: '9:30am daily', status: 'on',
          what: 'Proves the 7am Daily Sweep actually ran. A routine cannot be trusted to report its own absence, so something outside it has to check. Since 1 Sep 2026 an alarm goes to its log and a Mac notification only — no Slack.' },
        ...
        { key: 'inbound-triage', agent: true, name: 'Inbox Triage', when: '9am, 1pm, 5pm', status: 'on',
          what: 'Runs the Inbox Triage agent, which has its own register row above. Listed here so every scheduled job is accounted for.' },
        { key: 'task-manager', agent: true, name: 'Task Board Manager', when: '9:20am, 1:20pm, 5:20pm', status: 'on',
          what: 'Runs the Task Board Manager agent, which has its own register row above. Listed here so every scheduled job is accounted for.' },
        ...
        { key: 'prospecting', agent: true, name: 'Lead Finder', when: '9:15am daily', status: 'on',
          what: 'Runs the Lead Finder agent, which has its own register row above. Listed here so every scheduled job is accounted for.' },
        { key: 'prod-sweep-weekly', agent: true, name: 'Site Tester', when: 'Sundays 11:00am', status: 'on',
          what: 'Runs the Site Tester agent, which has its own register row above. Listed here so every scheduled job is accounted for.' },
        { key: 'uc-check', agent: true, name: 'UC Verification', when: '8:00am daily', status: 'off',
          what: 'Runs the UC Verification agent, which has its own register row above. RETIRED 1 Sep 2026: Kevin stopped the whole UC check process \u2014 missed payments are handled as arrears instead.' },
        ...
        { key: 'payment-run-daily', name: 'Payment Run daily check', when: '6:30am daily', status: 'on',
          what: 'Keeps the Payment Run list true between Fridays. It puts the payment cards you approved on the list on the Friday before they are due, marks a bill Paid once the bank shows a payment that names the payee, and lists any money that left the business account without ever being on the list. No AI, so it costs nothing to run.' },
    ...
    workers: [
        ...
        { name: 'contractor-bot', agent: true, when: 'every minute', status: 'on',
          what: 'Since 1 Sep 2026 the every-minute timer sends you ONE approvals digest at 8am (count plus a link to the dashboard queue, silent when nothing waits) and still posts Mica\u2019s lane cards to her DM and reads her emoji back. Your per-task cards, the contractor chat and the CEO DM chat are retired \u2014 the webhook now only turns replies under task DMs into task comments. Do not remove this cron: it carries the 8am digest and Mica\u2019s cards.' },
    ],
