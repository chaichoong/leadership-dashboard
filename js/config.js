...
// ══════════════════════════════════════════
    ...
    const PAGE_REGISTRY = [
        ...
        { id: 'os-strategy', name: 'Objective & Strategy',           icon: '🎯', pageVer: '1.63', sopFile: 'os/strategy/sop.html',       sopVer: '1.1', standalone: 'os/strategy/index.html' },
        ...
        { id: 'growth-plan', name: 'Growth Plan',                    icon: '📈', pageVer: '1.54', sopFile: 'sop-growth-plan.html',        sopVer: '1.5', standalone: 'growth-plan.html' },
        ...
        { id: 'agents', name: 'AI Agents', icon: '🤖', pageVer: '1.119', sopFile: 'sop-ai-agents.html', sopVer: '1.113', standalone: 'os/agents/index.html', adminOnly: true },
        ...
        { id: 'os-team',    name: 'Team Members',                  icon: '👥', pageVer: '1.40', sopFile: '',                            sopVer: '1.1', standalone: 'os/team/index.html' },
        ...
        { id: 'publishing', name: 'Publishing',                     icon: '📅', pageVer: '1.6', sopFile: '',                            sopVer: '1.0', standalone: 'publishing.html' },
        ...
        { id: 'skills',     name: 'Skills Library',                icon: '🧠', pageVer: '1.27', sopFile: 'guides/skills.html',           sopVer: '1.0', standalone: 'index.html#skills' },
    ...
    const TABLES = {
        ...
        aiAgents:      'tbl9msVjyQWslLOIZ', // AI Agents — role-agent register (Leadership → AI Agents page, os/agents/)
        ...
        estateStatus:       'tblZVrdzivyBueZVf', // Estate Status — one row per scheduled job (growth-plan.html reads the tenant-chain row; os/agents and publishing.html keep their own ESTATE_TBL)
        tenantLeads:        'tbliYKA44VBFeLduP', // Tenant Leads — over-35 UC tenant sign-ups, worked daily by scripts/tenant-leads.py (25 Sep 2026)
    };
    ...
    const GP = {
        ...
        prop: {
            ...
            strategy: 'fldivZ9UbAACwv7Yh', plannedExtra: 'fldFd4scZaJsXQ0n7', // Growth Strategy (Single let / UC joint tenancy / UC HMO / Serviced accommodation / Leave as is; renamed 16 Sep 2026, old values still read), Planned Extra Tenants (Kevin's per-house call, 9 Sep 2026)
            ...
            baselineRent: 'fldfTtL7On1C2OmRU', baselineDate: 'fldp0bTV5uUIQkHh6', baselineCt: 'fldFyN175n3TngNtt', // Growth Baseline Council Tax: frozen once with the rent (16 Sep 2026)
        ...
        // The tenant-finding chain monitor (Kevin only, 25 Sep 2026). scripts/tenant-leads.py runs daily at
        // 08:10 and writes ONE row to Estate Status (TABLES.estateStatus) with Key tenant-chain. Field IDs
        // below; the payload is a JSON string of the steps, openings and lead counts.
        chain: {
            rowKey: 'tenant-chain',
    ...
    const TASK_FIELDS = {
        ...
        // Ticking rememberThis is all a decision surface does. The lesson is
        // written by scripts/agent-dispatch.py lessons into the agent's own
        // definition file (~/.claude/agents/<agent>.md), which IS its prompt,
        // so a stored lesson is read on the agent's next run whether or not
        // anything remembers to inject it.
        rememberThis:    'fldZurhdHutYIDKVx', // checkbox — Kevin chose "reject and remember"
        verdictReason:   'fldF9Bs4N5mttQvtl', // singleSelect — WHY, so the lesson routes and accuracy is honest
        lessonWrittenAt: 'fldFfzXOME9Rh8SyM', // dateTime — idempotency stamp, written by the script only
        feedbackHistory: 'fldOzsq68lhfprKJu', // append-only archive; approvalFeedback is cleared on each resubmit
        // ── Knock it back a week (28 Aug 2026) ──
        // Kevin's own words: a confirmation statement he cannot file until an
        // authentication code arrives in the post should not sit in the
        // approval gate for a week looking like a decision he is dodging.
        // Setting this date keeps the task at Status Approval with all the
        // agent's work intact, and every approval surface hides it until the
        // date arrives. There is NO job that brings it back: each surface
        // filters on the date, so the day it passes the task simply reappears
        // everywhere at once. A cron that has to remember is a cron that can
        // forget (see feedback_a_running_job_is_not_a_working_job).
