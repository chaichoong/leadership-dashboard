---
name: prod-e2e-sweep
description: Daily end-to-end browser sweep of the leadership dashboard: walks every tab, checks render + console + a read-only action, verifies prod is live, Slack-DMs Kevin and files an Airtable task only on failure.
---
...
```
python3 /Users/kevinbrittain/Projects/leadership-dashboard/scripts/job-queue.py acquire prod-e2e-sweep --lease 90
```
...
```
python3 /Users/kevinbrittain/Projects/leadership-dashboard/scripts/job-queue.py release prod-e2e-sweep
```

If your run will take longer than 90 minutes, extend the lease as you go:
`python3 /Users/kevinbrittain/Projects/leadership-dashboard/scripts/job-queue.py heartbeat prod-e2e-sweep --lease 90`.
