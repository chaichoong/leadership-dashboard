...
3. ROUTING — two lanes since 24 Aug 2026:
   ...
   a. DETERMINISTIC (no CEO dispatch): any routingNeeded task carrying `autoTarget` is an inbound reply task and its destination is fixed (Kevin's ruling): apply it directly with python3 scripts/agent-dispatch.py route TASKID --to <autoTarget>. No judgement call, no od-ceo cost. An INBOUND task WITHOUT autoTarget means the Response agent's register row is not Built/Live (Kevin's pause lever) or the register was unreadable — treat it as an ordinary CEO-lane task and never force the route; the script refuses routes to a paused role agent anyway.
