...
- 2026-07-23 (CLOUDFLARE EMAIL OBFUSCATION): Portman Stone's privacy policy renders its address as `[email protected]` linking to `/cdn-cgi/l/email-protection` — the address is JavaScript-decoded, so neither WebFetch nor Chrome `get_page_text` can read it. Do not try to decode it and do not guess. "No published email" is a legitimate outcome: route to Website contact form and say so.
