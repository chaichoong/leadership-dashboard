...
// Drive the real cmd_reconcile with get_task stubbed and STATE_DIR pointed
...
m.STATE_DIR = ${JSON.stringify(root)}
...
// ─── THE COMMAND MUST BE REACHABLE FROM THE CLI ──────────────────────
//
// Everything above drives cmd_reconcile directly, by importing the module. That
// is the right way to test the LOGIC and it is also blind to the actual bug
// (20260824-agent-dispatch-336): the function is not what was missing —
// argparse was. SKILL.md ordered `agent-dispatch.py reconcile` as step 1 of
// every run from 19 Aug, argparse answered "invalid choice: reconcile", the run
// logged it and carried on, and the control never once ran.
//
// Proved necessary: reverting only the dispatch-table entry left every test
// above GREEN while the command was unreachable again.
describe('reconcile is wired into the command line, not just defined', () => {
  it('argparse accepts "reconcile" as a choice', () => {
    const r = spawnSync('python3', [DISPATCH, "reconcile", "--help"], { encoding: 'utf8' });
    expect(r.stderr || '').not.toMatch(/invalid choice/);
    expect(r.status).toBe(0);
  });

  it('--runs is accepted, so the skill can scope the sweep', () => {
    const r = spawnSync('python3', [DISPATCH, "reconcile", "--help"], { encoding: 'utf8' });
    expect(r.stdout).toMatch(/--runs/);
  });

  it('main() RETURNS the handler exit code instead of discarding it', () => {
    // reconcile reports by RETURNING 1. Before this, main() called the handler
    // and threw the result away, so every command exited 0 whatever it found
    // and the whole check would have been ornamental.
    const src = readFileSync(DISPATCH, 'utf8');
    expect(src).toMatch(/sys\.exit\(main\(\)\)/);
    expect(src).toMatch(/return \{"queue": cmd_queue/);
  });
});
