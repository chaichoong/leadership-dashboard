...
// finances two Swinton policies under a single agreement, so "REF 80000069" appears on BOTH
...
const LOOKUP = {
  kr4ap: { id: 'kr4ap', fields: { [FIELDS.costName]: 'Kent Reliance - 4AP - MOM0100401BRI1', [FIELDS.costExpected]: 695.50 } },
  kr34cr: { id: 'kr34cr', fields: { [FIELDS.costName]: 'Kent Reliance - 34CR - MOM0100402BRI1', [FIELDS.costExpected]: 686.67 } },
  rsap: { id: 'rsap', fields: { [FIELDS.costName]: 'Swinton RSAP6800000100 - Ref 80000069', [FIELDS.costExpected]: 42.01 } },
  be26: { id: 'be26', fields: { [FIELDS.costName]: 'Swinton BE26ACTP000000010001 - Ref 100000062', [FIELDS.costExpected]: 45.30 } },
};
...
const REF_INDEX = {
  MOM0100401BRI1: ['kr4ap'],            // unique to one mortgage
  MOM0100402BRI1: ['kr34cr'],           // unique to one mortgage
  '80000069': ['rsap', 'be26'],         // Close Brothers agreement — serves BOTH policies
};
...
test.describe('Reconciliation: pick the cost by reference', () => {
  ...
  test('a unique reference identifies the mortgage exactly', async ({ page }) => {
    expect(await byRef(page, 'DIRECT DEBIT PAYMENT TO KENT RELIANCE IP REF MOM0100401BRI1, MANDATE NO 0100')).toBe('kr4ap');
    expect(await byRef(page, 'DIRECT DEBIT PAYMENT TO KENT RELIANCE IP REF MOM0100402BRI1, MANDATE NO 0100')).toBe('kr34cr');
  });
  ...
  test('the reference wins even when the amount has drifted far from Expected', async ({ page }) => {
    ...
    expect(await byRef(page, 'KENT RELIANCE IP REF MOM0100401BRI1')).toBe('kr4ap');
  });

  test('THE GUARD: a shared reference must NOT decide the match', async ({ page }) => {
    // "80000069" is the Close Brothers agreement, seen against BOTH Swinton policies.
    ...
    expect(await byRef(page, 'DIRECT DEBIT PAYMENT TO CLOSE-SWINTON REF 80000069, MANDATE NO 0207')).toBe('');
  });
  ...
  test('two discriminating references that disagree abstain', async ({ page }) => {
    expect(await byRef(page, 'REF MOM0100401BRI1 AND ALSO MOM0100402BRI1')).toBe('');
  });
  ...
  test('short mandate numbers are not treated as references', async ({ page }) => {
    ...
    const tokens = await page.evaluate(() => window.extractRefTokens('DIRECT DEBIT REF 80000069, MANDATE NO 553'));
    expect(tokens).toContain('80000069');
    expect(tokens).not.toContain('553');
  });

  test('reference extraction ignores ordinary words', async ({ page }) => {
    const tokens = await page.evaluate(() => window.extractRefTokens('DIRECT DEBIT PAYMENT TO KENT RELIANCE IP REF MOM0100401BRI1'));
    expect(tokens).toEqual(['MOM0100401BRI1']);
  });
});
