...
test.describe('Objective & Strategy — starting a new quarter', () => {
    ...
    test('a plan whose only content is a linked method step still saves', async ({ page }) => {
        // saveRecord writes the Main Method link slots into `fields` itself —
        // the pickers are not [data-field-id] inputs, so readAllFormFields
        // cannot see them. That means the empty-create guard has to run AFTER
        // that block, or linking a method step and nothing else reads as an
        // empty plan and gets blocked.
        //
        // The other half of that ordering is the guard's Array.isArray clause:
        // unselected slots are written as [], and an empty array is neither
        // null nor '', so without it every blank form would look like content
        // and the junk record would sail through.
        const writes = await loadStrategyPage(page);

        await page.locator('.empty-state').getByRole('button', { name: /Fill in manually/i }).click();
        await page.waitForTimeout(400);

        // Pick a method step, touching nothing else. renderForm resets
        // methodLinkSel, so set it after the form exists.
        const linkFid = await page.evaluate(() => {
            methodLinkSel[0] = 'recMethodStep1';
            markDirty();
            return OBJSTRAT.mainMethodLinks[0];
        });
        ...
        const planWrites = writes.filter(w => w.url.includes(OBJ_STRAT_TABLE) && w.method === 'POST');
        expect(planWrites).toHaveLength(1);
        expect(planWrites[0].body.fields[linkFid]).toEqual(['recMethodStep1']);
    });
});
