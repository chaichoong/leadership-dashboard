...
const tenants = [['tAcme', 'ACME DEMO LTD'], ['tBloggs', 'John Bloggs'], ['tExampleM', 'Mary Example'], ['tExampleA', 'Arthur Example'], ['tDummy', 'Dummy Lettings'], ['tTester', 'Tom Tester'], ['tAgent', 'Brackenfold']];
const tenancies = [
  ['ten32_1', 'tAcme', 'DEMO LTD', 'u32_1', 350], ['ten32_2', 'tAcme', 'DEMO LTD', 'u32_2', 350], ['ten32_3', 'tAcme', 'DEMO LTD', 'u32_3', 350],
  ['ten32_4', 'tAcme', 'DEMO LTD', 'u32_4', 350], ['ten32_5', 'tAcme', 'DEMO LTD', 'u32_5', 350],
  ['tenBloggs', 'tBloggs', 'Bloggs', 'u42_1', 524.6],
  ['tenExampleM', 'tExampleM', 'Example', 'uD_1', 1296.45], ['tenExampleA', 'tExampleA', 'Example', 'uD_3', 897.52],
  ['tenDummy3', 'tDummy', 'Lettings', 'uDk_3', 651], ['tenDummy9', 'tDummy', 'Lettings', 'uDk_9', 641.2],
  ['tenTester', 'tTester', 'Tester', 'u42_1', 524.9],
  ['tenAgent', 'tAgent', 'Brackenfold', 'uD_1', 1105.5],
];
const costs = [
  ['c32', 'Kent Reliance - 32EP - 70000144', P.elm32, 660.69, 'In Payment'],
  ['c42', 'Kent Reliance - 42EP - MOM0100403BRI1', P.elm42, 724.07, 'In Payment'],
  ['cBM', 'Birmingham Midshires - 5DP - 60900000000500', P.dalham, 223.33, 'In Payment'],
  ['cOld', 'Kent Reliance - OLD - 70000999', P.elm32, 500, 'Inactive'],
  ['cSwA', 'Swinton - Policy RSAP68 - Ref 80000069', P.dalham, 42.01, 'In Payment'],
  ['cSwB', 'Swinton - Policy BE26AC - Ref 100000062', P.dalham, 45.3, 'Overdue'],
];
...
describe('findPropertiesInText', () => {
  it('reads the short code, number + street, and a unique street word', () => {
    expect(ctx.findPropertiesInText('REF.32EP-LANDLORD-PAY FROM ACME DEMO LTD', idx)).toEqual([P.elm32]);
    expect(ctx.findPropertiesInText('John Sample 5 Dalham', idx)).toEqual([P.dalham]);
  ...
  it('does not pick a property from a street word shared by several (ELMDON)', () => {
    expect(ctx.findPropertiesInText('JOHN SAMPLE REFERENCE ELMDON', idx)).toEqual([]);
  });
  it('returns every property named, so callers can refuse an ambiguous one', () => {
    expect(ctx.findPropertiesInText('John Sample 32 Elmdon 42 Elmdon', idx).sort()).toEqual([P.elm32, P.elm42].sort());
  });
});

describe('matchTenancyDirect', () => {
  it('finds a tenancy from the surname in a bank giro credit', () => {
    expect(ctx.matchTenancyDirect('BANK GIRO CREDIT REF BLOGGS, 000000001000000204', '', 524.6, idx)).toEqual({ tenancyId: 'tenBloggs' });
  });
  it('never links an outgoing payment to a tenancy', () => {
    expect(ctx.matchTenancyDirect('BILL PAYMENT TO JOHN BLOGGS REFERENCE 42 ELMDON', 'JOHN BLOGGS', -90, idx)).toBeNull();
  });
  it('splits two tenants who share a surname by rent', () => {
    expect(ctx.matchTenancyDirect('BANK GIRO CREDIT REF EXAMPLE', '', 1296.45, idx)).toEqual({ tenancyId: 'tenExampleM' });
    expect(ctx.matchTenancyDirect('BANK GIRO CREDIT REF EXAMPLE', '', 897.52, idx)).toEqual({ tenancyId: 'tenExampleA' });
  });
  it('uses the apartment number in the descriptor, not the rent, to pick the unit', () => {
    ...
    expect(ctx.matchTenancyDirect('DUMMY LETTINGS LTDRENT APARTMENT 3', '', 641.2, idx)).toEqual({ tenancyId: 'tenDummy3' });
  });
  it('reads the split position as the unit when one landlord pays several units', () => {
    expect(ctx.matchTenancyDirect('FASTER PAYMENTS RECEIPT REF.32EP-LANDLORD-PAY FROM ACME DEMO LTD -- ACME DEMO LTD (Split 4 of 5)', 'ACME DEMO LTD', 382.7, idx)).toEqual({ tenancyId: 'ten32_4' });
  });
  it('returns the candidates, not a guess, when the tenant is known but the unit is not', () => {
    const m = ctx.matchTenancyDirect('FASTER PAYMENTS RECEIPT REF.32EP-LANDLORD FROM ACME DEMO LTD', 'ACME DEMO LTD', 1750, idx);
  ...
  it('finds nothing when no active tenant is named', () => {
    expect(ctx.matchTenancyDirect('BANK GIRO CREDIT REF GENERIC SUPPORT LIMITED, S10000000000000026', '', 320, idx)).toBeNull();
  });
});

describe('matchCostDirect', () => {
  it('finds the mortgage from the account number the cost is named after, with no history', () => {
    expect(ctx.matchCostDirect('DIRECT DEBIT PAYMENT TO ONESAVINGS BANK REF 70000144, MANDATE NO 0111', -660.69, idx, {})).toBe('c32');
    expect(ctx.matchCostDirect('DIRECT DEBIT PAYMENT TO KENT RELIANCE IP REF MOM0100403BRI1, MANDATE NO 0153', -724.07, idx, {})).toBe('c42');
  });
  it('accepts a bank-padded account number', () => {
    expect(ctx.matchCostDirect('DIRECT DEBIT PAYMENT TO BHAM MIDSHIRES REF 6090000000050000, MANDATE NO 0166', -223.33, idx, {})).toBe('cBM');
  });
  it('never proposes an inactive cost', () => {
    expect(ctx.matchCostDirect('DIRECT DEBIT PAYMENT TO ONESAVINGS BANK REF 70000999', -500, idx, {})).toBe('');
  });
  it('defers to history when the reference is known to serve two costs (Close Brothers / Swinton)', () => {
    const refIndex = { '80000069': new Set(['cSwA', 'cSwB']) };
    expect(ctx.matchCostDirect('DIRECT DEBIT PAYMENT TO CLOSE-SWINTON REF 80000069, MANDATE NO 0207', -45.3, idx, refIndex)).toBe('');
  });
  it('keeps a direct debit reversal on the cost it reverses, even though the money is incoming', () => {
    expect(ctx.matchCostDirect('DIRECT DEBIT REVERSAL REF 70000144, MANDATE NO 0111', 660.69, idx, {})).toBe('c32');
    expect(ctx.matchCostDirect('BANK GIRO CREDIT REF 70000144', 660.69, idx, {})).toBe('');
  });
});
...
describe('runReconciliationMatching with the direct layer', () => {
  ...
  it('overrides a vendor-keyed knowledge-base rule that points at the wrong mortgage', () => {
    ...
    ctx.set('allTransactions', [tx('t1', 'DIRECT DEBIT PAYMENT TO ONESAVINGS BANK REF 70000144, MANDATE NO 0111 -- ONESAVINGS BANK', 'ONESAVINGS BANK', -660.69)]);
  ...
  it('overrides the generic "bank giro|credit ref" history key with the named tenant', () => {
    const hist = tx('h1', 'BANK GIRO CREDIT REF TESTER, 000000001000000561', '', 524.9, { [F().txReconciled]: true, [F().txTenancy]: ['tenTester'], [F().txUnit]: ['u42_1'], [F().txSubCategory]: [ctx.REC.subRentalInc], [F().txCategory]: ['catRev'], [F().txBusiness]: ['bizRE'] });
    ctx.set('allTransactions', [hist, tx('t2', 'BANK GIRO CREDIT REF BLOGGS, 000000001000000204', '', 524.6)]);
    const r = ctx.runReconciliationMatching().find(x => x.txId === 't2');
    expect(r.tenancyId).toBe('tenBloggs');
  ...
  it('clears a tenancy proposed by the generic "bank giro|credit ref" key when no active tenant is named', () => {
    const hist = tx('h1', 'BANK GIRO CREDIT REF TESTER, 000000001000000561', '', 524.9, { [F().txReconciled]: true, [F().txTenancy]: ['tenTester'], [F().txUnit]: ['u42_1'], [F().txSubCategory]: [ctx.REC.subRentalInc] });
    ctx.set('allTransactions', [hist, tx('t3', 'BANK GIRO CREDIT REF GENERIC SUPPORT LIMITED, S10000000000000026', '', 320)]);
  ...
  it('keeps a tenancy learned from the identical descriptor when rent is paid by a third party', () => {
    ...
    const uc = 'BANK GIRO CREDIT REF QQ123456C DWP UC, 000000001000000900';
    const hist = tx('h2', uc, '', 524.9, { [F().txReconciled]: true, [F().txTenancy]: ['tenTester'], [F().txUnit]: ['u42_1'], [F().txSubCategory]: [ctx.REC.subRentalInc] });
    ...
    expect(r.tenancyId).toBe('tenTester');
  });
  ...
  it('a bare street word fills a blank property but does not override a learned one', () => {
    ...
    ctx.set('allTransactions', [tx('t7', 'OLDHAM COUNCIL TAX', 'OLDHAM COUNCIL', -20), tx('t8', 'Sid Nobody Oldham', 'Sid Nobody Oldham', -66), tx('t9', 'OLDHAM COUNCIL 1406 OLDHAM', 'OLDHAM COUNCIL', -20)]);
  ...
  it('a first name alone does not identify a tenancy', () => {
    expect(ctx.matchTenancyDirect('FASTER PAYMENTS RECEIPT REF.REFUND FROM JOHN SAMPLE', 'JOHN SAMPLE', 90, idx)).toBeNull();
  });

  it('ignores the split position when the candidates are different tenants', () => {
    // Two Examples at different units: "(Split 1 of 2)" must not pick unit 1 by position.
    expect(ctx.matchTenancyDirect('BANK GIRO CREDIT REF EXAMPLE (Split 1 of 2)', '', 897.52, idx)).toEqual({ tenancyId: 'tenExampleA' });
  });

  it('compares unit numbers numerically', () => {
    expect(ctx.matchTenancyDirect('DUMMY LETTINGS LTDRENT FLAT 03', '', 641.2, idx)).toEqual({ tenancyId: 'tenDummy3' });
  });

  it('the rent tiebreaker no longer undoes a unit-number match', () => {
    ctx.set('allTransactions', [tx('t4', 'DUMMY LETTINGS LTDRENT APARTMENT 3', '', 641.2)]);
    const [r] = ctx.runReconciliationMatching();
    expect(r.tenancyId).toBe('tenDummy3');
  });

  it('sets the property from the address when nothing more specific applies', () => {
    ctx.set('allTransactions', [tx('t5', 'John Sample 5 Dalham', 'John Sample 5 Dalham', -90)]);
    const [r] = ctx.runReconciliationMatching();
