...
// DEPLOYED CONTRACT — read before replacing this worker:
// This proxy holds the Anthropic key server-side (CLAUDE_API_KEY secret) and
// authenticates callers by browser origin+Sec-Fetch OR service bearer tokens
// (PROXY_SERVICE_TOKEN = GitHub valuations job, PROXY_SERVICE_TOKEN_2 =
// agent-runner via a Cloudflare service binding). Deploying a version without
// these breaks every AI feature in the app AND the agent runtime.
...
export default {
  async fetch(request, env) {
    const corsHeaders = {
      'Access-Control-Allow-Origin': ALLOWED_ORIGINS.includes(request.headers.get('Origin') || '') ? request.headers.get('Origin') : ALLOWED_ORIGIN,
      'Vary': 'Origin',
