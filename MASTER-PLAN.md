...
- [ ] ERICAMAE — (NEW 13 Jul; amended same day) Nurture sequence triggers on GHL tag `od-prospect-nurture`; **3 emails maximum** (Kevin's call: non-aggressive, value-led, each with a call CTA); templates carry sender identity, postal address and unsubscribe (PECR) with Kevin sign-off before switch-on (done when: test contact tagged receives the 3-email sequence; due 18 Jul) [AT:-]
