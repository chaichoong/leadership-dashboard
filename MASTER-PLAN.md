...
| Date | Source | Change |
|---|---|---|
| 2026-07-22 | Kevin, in-session (approved plan) | §6 client intake: shipped the **public client onboarding form** (`onboarding.html`) — a signed-up client fills essential account-setup answers (identity, businesses, locale, team, banking, targets, act/ask thresholds); on submit the `onboarding-submit` Edge Function (service-role, `--no-verify-jwt`, honeypot-guarded) saves the full answers to the new `onboarding_submissions` table and creates a CRM contact + a **Won**-stage deal in the Runpreneur workspace, chaining into the existing "Create client account" provisioning button. Migration `0032_onboarding.sql` + function await Mica's deploy (`db push` + `functions deploy`). Advances §6 line "Async client intake form" [AT:recukdbjiqU5UejBo] → `[~]`; still owes the Loom walkthrough + a real test submission post-deploy. CRM gains a "📋 Onboarding link" copy button. Branch `feature/onboarding-form`. |
